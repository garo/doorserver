...
var UsbKeyboard = module.exports = function (cfg) {
  this.buffer = "";
  this.cfg = cfg;
  this.on('rawtoken', this.onRawToken.bind(this));
...
UsbKeyboard.prototype.onRawToken = function (event) {
  console.log("onRawToken", event);
  var token = null;

  if (this.cfg.type && this.cfg.type == 'hexinput') {
    token = this.convertHexInput(event.rawtoken);
  } else {
...
UsbKeyboard.prototype.convertHexInput = function (hex) {
  var str = parseInt(hex, 16).toString();
  str = strPad(str, 10, '0');
