...
exports.findUserByToken = function (token, cb) {
  doorserver.repositories.mysql.fetchHandle("data", function (err, handle) {
    handle.query("SELECT id,name FROM doorserver_users JOIN doorserver_keys ON (doorserver_keys.uid = doorserver_users.id) WHERE token = ? AND doorserver_users.enabled = 1 AND doorserver_keys.enabled = 1", [token], function (err, rows) {
      if (err) {
        cb(err);
        return;
      }
