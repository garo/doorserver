    ...
    if (user) {
      doorserver.services.security.isUserAllowedToOpenDoor(user, token_packet.door_id, function (err, canOpen) {
        ...
        if (canOpen) {
          ...
          exports.logDeniedUser(user, token_packet.door_id, token_packet.token, "not allowed to open door (no valid user group)", cb);
        }
      });
    } else {
      exports.logDeniedUser(user, token_packet.door_id, token_packet.token, "unknown user or token or disabled user", cb);
    }
  });
...
exports.logDeniedUser = function(user, door_id, token, reason, cb) {
  console.log((new Date().toISOString()) + " User", user, "with token", token, "access was denied to door", door_id, "reason:", reason);
...
exports.logAllowedUser = function(user, door_id, token, cb) {
  console.log((new Date().toISOString()) + " User", user, "with token", token, "access was granted to door", door_id);
