...
exports.isDoorOpen = function(door_id) {
  if (exports.doorHoldState[door_id] === undefined) {
    ...
    if (doorserver.settings.get("doors")[door_id]) {
      exports.doorHoldState[door_id] = exports.DOOR_CLOSED;
    } else {
...
exports.openDoor = function (door_id, cb) {
  ...
  if (exports.doorHoldState[door_id] !== exports.DOOR_OPEN) {
    console.log("Opened door", door_id);
  }

  exports.doorHoldState[door_id] = exports.DOOR_OPEN;
  cb();
};
...
exports.closeDoor = function (door_id, cb) {
  ...
  var door_settings = doorserver.settings.get("doors")[door_id];
  if (!door_settings) {
    console.warn((new Date().toISOString()) + " Unknown door", door_id);
    return;
  }

  if (!exports.doorTemporarilyOpen[door_id]) {

    // Close door
    doorserver.drivers.piface.off(door_settings.relay_pin);
    if (exports.doorHoldState[door_id] !== exports.DOOR_CLOSED) {
      console.log("Closed door", door_id);
    }
  }
...
exports.openDoorForAMoment = function (door_id) {
  ...
  if (!exports.isDoorOpen(door_id)) {

    // Mark that the door is temporarily open
    exports.doorTemporarilyOpen[door_id] = new Date();
    ...
    setTimeout(function () {
      ...
      delete exports.doorTemporarilyOpen[door_id];
      doorserver.drivers.piface.off(door_settings.relay_pin);
    }, door_settings.door_open_time);

    console.log((new Date().toISOString()) + " Opening door", door_id, "for", (door_settings.door_open_time / 1000), "seconds");
  } else {
    console.log((new Date().toISOString()) + " Asked to open door", door_id, "but it was already open");
  }
