 ...
 * @param door Door model
 */
exports.openDoorForAMoment = function (door, duration) {
  ...
  if (!duration) {
    duration = door_settings.door_open_time;
  }
  ...
  if (!exports.isDoorOpen(door)) {
    ...
    console.log((new Date().toISOString()) + " Opening door", JSON.stringify(door), "for", (duration / 1000), "seconds");
  } else {
