 ...
 *
 * This is done by first checking if any of the inclusion rules match. First matching
 * rule stops the evaluation and the evaluation result is proposed to be true.
 *
...
Timeperiod.check_time_period = function (date, period) {
  ...
  if (m) {
    ...
    for (var i = d1; i <= d2; i++) {
      date1 = moment(date).day(i).hour(h1).minute(m1).second(0).toDate();
      date2 = moment(date).day(i).hour(h2).minute(m2).second(59).toDate();
